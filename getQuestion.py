...
def save_one(file, topic, law, title, a, b, c, d, answer):
    db = pymysql.connect(host="localhost", user="root", password="root", database="law_enforcement")
    cursor = db.cursor()
...
def save_all(file):
    ...
    with open('d:/执法证/题库/' + file + '.txt', 'r', encoding='UTF-8') as f:
...
def analysis():
    ...
    with open('d:/执法证/' + topic_list[0] + '--统计.txt', 'w', encoding='utf8') as f:
        f.write("\tA\tB\t\n")
        for law in law_list:
            f.write(law + '\t')
            for answer in ['A', 'B']:
            ...
            f.write('\n')
    print('导出文件-->d:/执法证/' + topic_list[0] + '--统计.txt！')

    with open('d:/执法证/' + topic_list[1] + '--统计.txt', 'w', encoding='utf8') as f:
        f.write("\tA\tB\tC\tD\t\n")
        for law in law_list:
            f.write(law + '\t')
            ...
            f.write('\n')
    print('导出文件-->d:/执法证/' + topic_list[1] + '--统计.txt！')

    with open('d:/执法证/' + topic_list[2] + '--统计.txt', 'w', encoding='utf8') as f:
        ...
        for law in law_list:
            ...
            f.write('\n')
    print('导出文件-->d:/执法证/' + topic_list[2] + '--统计.txt！')
...
def get_question(session, file):
    ...
    with open('d:/执法证/题库/' + file.replace('/', '-') + '.txt', 'w', encoding='utf8') as f:
        f.write(request.text)
    print('d:/执法证/题库/' + file.replace('/', '-') + '.txt--下载完成')


def get_questions(session):
    file = open('d:/执法证/题库/下载目录.txt', "r", encoding='utf-8')
    line = file.readline()
    while line:
...
def readExam(file):
    ...
    with open('d:/执法证/题库/' + file + '.txt', 'r', encoding='UTF-8') as f:
        ...
        for name in tfng_list:
            ...
            db = pymysql.connect(host="localhost", user="root", password="root", database="law_enforcement")
            cursor = db.cursor()
            cursor.execute("SELECT answer FROM question_one WHERE title='" + title + "'")
            results = cursor.fetchone()
            answer = results[0]
            print(str(i) + topic + '：' + answer + '--' + title)
            i = i+1
        for name in single_list:
            topic = '单选题'
            law = re.findall(r'\[(.+?)\]', BeautifulSoup(str(html.find('div', {'id': name})), "html.parser").find('div', {'class': 'exam-courseName'}).string)[0]
            ...
            db = pymysql.connect(host="localhost", user="root", password="root", database="law_enforcement")
            cursor = db.cursor()
            cursor.execute("SELECT answer FROM question_one WHERE title='" + title + "' AND a='" + a + "'")
            results = cursor.fetchone()
            answer = results[0]
            print(str(i) + topic + '：' + answer + '--' + title)
            i = i + 1
